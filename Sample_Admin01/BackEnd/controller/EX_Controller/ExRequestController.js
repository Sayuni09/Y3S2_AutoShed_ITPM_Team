...
exports.submitRescheduleRequest = async (req, res) => {
    const { schedule_id, examiner_id, comment } = req.body;
    
    if (!schedule_id || !examiner_id || !comment) {
        return res.status(400).json({ 
            success: false, 
            message: 'Missing required fields: schedule_id, examiner_id, and comment are required'
        });
    }
    ...
    try {
        ...
        const query = `
            INSERT INTO examiner_rescheduling_requests (schedule_id, examiner_id, comment)
            VALUES (?, ?, ?)
        `;
        
        const [result] = await db.promise().execute(query, [schedule_id, examiner_id, comment]);
...
exports.getExaminerRescheduleRequests = async (req, res) => {
    ...
    try {
        // Query to get all reschedule requests with schedule details for an examiner
        const query = `
            SELECT err.request_id, err.schedule_id, err.comment, err.status, 
                   err.admin_response, err.requested_at, err.responded_at,
            ...
            WHERE err.examiner_id = ?
            ORDER BY err.requested_at DESC
        `;
        
        const [requests] = await db.promise().execute(query, [examiner_id]);
...
exports.getRescheduleRequestById = async (req, res) => {
    ...
    try {
        ...
        const query = `
            SELECT err.request_id, err.schedule_id, err.examiner_id, err.comment, 
                   err.status, err.admin_response, err.requested_at, err.responded_at,
                   vs.module_code, DATE_FORMAT(vs.date, '%Y-%m-%d') AS date, 
                   vs.start_time, vs.end_time, 
                   vs.venue_name, vs.viva_type,
                   m.module_name,
            ...
            JOIN viva_schedules vs ON err.schedule_id = vs.schedule_id
            LEFT JOIN modules m ON vs.module_code = m.module_code
            LEFT JOIN examiners e ON err.examiner_id = e.examiner_id
