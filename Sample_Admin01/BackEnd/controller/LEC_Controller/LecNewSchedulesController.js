...
// Fetch schedules for a lecturer with detailed information
exports.getLecturerSchedules = (req, res) => {
    ...
    const query = `
        ...
        LEFT JOIN schedule_acceptance sa 
            ON vs.schedule_id = sa.schedule_id AND sa.lec_id = ?
        LEFT JOIN reschedule_requests rr 
            ON vs.schedule_id = rr.schedule_id AND rr.lec_id = ?

        WHERE sl.lec_id = ?
        AND sa.schedule_id IS NULL
        AND rr.schedule_id IS NULL
        ORDER BY vs.date, vs.start_time, sbg.start_time`;

    db.query(query, [lec_id, lec_id, lec_id], (err, results) => {
        if (err) {
