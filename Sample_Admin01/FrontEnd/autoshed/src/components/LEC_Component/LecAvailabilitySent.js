...
import { getAvailability, deleteAvailability, updateAvailability } from '../../services/LEC_Services/LecAvailabilityService';
import '../../styles/LEC_Styles/LecAvailabilitySent.css';

const LecAvailabilitySent = () => {
  ...
  const fetchAvailabilityForms = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await getAvailability(lecturerId);
  ...
  }, [lecturerId, fetchAvailabilityForms]);
  ...
  const handleDelete = async (form_id) => {
    if (window.confirm('Are you sure you want to delete this availability form?')) {
      try {
        setLoading(true);
        await deleteAvailability(form_id);
              ...
              {editSlots.map((slot, index) => (
                <div key={index} className="slot-edit">
                  <div className="slot-edit-header">
                    ...
                    <input
                      type="date"
                      value={slot.available_date} // Displays the current available_date
                      onChange={(e) => {
                        const updatedSlots = [...editSlots];
                        updatedSlots[index].available_date = e.target.value; // Update the date if modified
...
export default LecAvailabilitySent;