...
import '../../styles/LEC_Styles/LecAcceptedSchedules.css';

const LecAcceptedSchedules = () => {
  ...
  // Fetch accepted schedules whenever component mounts or lecturer ID changes
  useEffect(() => {
    const getAcceptedSchedules = async () => {
      if (lecturerId) {
        try {
          setLoading(true);
          const data = await fetchAcceptedSchedules(lecturerId);
        ...
        setLoading(false);
        setError("Lecturer ID is required to fetch schedules");
      }
    };
    ...
    <div className="accepted-schedules-container">
      <div className="header">
        <div className="header-content">
          <h2>Accepted Lecture Schedules</h2>
        </div>
...
export default LecAcceptedSchedules;