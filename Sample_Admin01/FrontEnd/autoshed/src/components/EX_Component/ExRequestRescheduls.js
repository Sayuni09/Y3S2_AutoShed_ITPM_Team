...
import '../../styles/EX_Styles/ExRequestRescheduls.css';
import ExRequestService from "../../services/EX_Services/ExRequestService";

const ExRequestRescheduls = () => {
  ...
  // Extract examiner ID from JWT token on component mount
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      setLoading(true);
      try {
        const rescheduleRequests = await ExRequestService.getExaminerRescheduleRequests(examinerId);
  ...
  const getStatusClass = (status) => {
    if (status === 'approved') return 'status-badge accepted';
...
export default ExRequestRescheduls;
